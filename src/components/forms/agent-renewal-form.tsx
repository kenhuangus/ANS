...
export function AgentRenewalForm() {
  ...
  const form = useForm<AgentRenewalRequestPayload>({
    ...
    defaultValues: { // Placeholders, AI can fill if needed
      ansName: "",
      certificate: {
        subject: "", 
        issuer: "",
  ...
  async function handleAiFill() {
    ...
    if (!currentValues.ansName || currentValues.ansName.trim() === "") {
      toast({
        ...
        description: "Please enter the ANSName of the agent to renew before using AI assistance.",
        variant: "destructive",
      });
  ...
  async function onSubmit(data: AgentRenewalRequestPayload) {
    ...
    if (!data.ansName || !data.certificate?.pem || !data.certificate?.subject) {
       toast({
        ...
        description: "ANSName and a new CSR (Subject and PEM) are required for renewal. Use 'AI Fill' or complete them manually.",
        variant: "destructive",
      });
      setIsLoading(false);
      return;
  ...
  return (
    <Card className="w-full max-w-xl shadow-xl">
      <CardHeader>
        ...
        <CardDescription>
          Provide agent&apos;s ANSName and new CSR. Use &quot;AI Fill CSR&quot; for CSR generation.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex justify-end">
              <Button type="button" variant="outline" onClick={handleAiFill} disabled={isAiLoading || !form.getValues().ansName} className="mb-2">
